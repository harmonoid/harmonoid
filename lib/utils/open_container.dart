...
class OpenContainer<T extends Object?> extends StatefulWidget {
  ...
  /// The [navigatorKey] argument is used to determine the navigator to push the
  /// route for [openBuilder] to.
  ///
  /// If [navigatorKey] is not provided, the route created will push to the
  /// nearest navigator.
...
class _OpenContainerState<T> extends State<OpenContainer<T?>> {
  ...
  Future<void> openContainer() async {
    ...
    final T? data = await (widget.navigatorKey?.currentState ??
            Navigator.of(
              context,
...
class _OpenContainerRoute<T> extends ModalRoute<T> {
  ...
  void _takeMeasurements({
    ...
    final RenderBox navigator = (navigatorKey?.currentState ??
            Navigator.of(
              navigatorContext,
