...
abstract class YTM {
  ...
  static Future<Track?> identify(String query) async {
    ...
    try {
      ...
      var body = convert.jsonDecode(response.body)[2]['playerResponse']
          ['videoDetails'];
      var description = body["shortDescription"].split("\n\n");
      var trackArtistNames;
      var albumName;
      try {
        trackArtistNames = description[1]
            .split(" · ")
            .sublist(1)
            .map((meta) => meta.trim())
            .toList()
      ...
      return Track(
        trackId: id,
        trackName: body['title'],
        trackArtistNames: trackArtistNames,
        albumArtistName:
            trackArtistNames.isNotEmpty ? trackArtistNames.first : null,
...
extension TrackExtension on Track {
  Future<void> attachAudioStream({bool meta: false}) async {
    if (filePath != null) return;
    try {
      currentlyPlaying.state = language!.STRING_RETRIEVING_INFO;
      ...
      for (var format in body['streamingData']['adaptiveFormats']) {
      ...
      //
      currentlyPlaying.state = language!.STRING_RETRIEVING_LINK;
      ...
      currentlyPlaying.state = language!.STRING_STARTING_PLAYBACK;
    } catch (exception, stacktrace) {
      print(exception);
      print(stacktrace);
      currentlyPlaying.state = language!.STRING_NO_INTERNET_TITLE;
    }
  }
      ...
      try {
        ...
        var track = Track(
          ...
          albumArtistName: metas
              ...
              .first,
          trackDuration: duration * 1000,
          networkAlbumArt: object['thumbnail']['thumbnails'][2]['url'],
          albumId: object['menu']['menuRenderer']['items']
