...
class ArtistTrackViewer extends StatefulWidget {
  final Map<String, dynamic> artist;
  static String pageRoute = '/artisttrackviewer';
  
  ArtistTrackViewer({Key key, @required this.artist}) : super(key : key);
  _ArtistTrackViewer createState() => _ArtistTrackViewer();
}

class _ArtistTrackViewer extends State<ArtistTrackViewer> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    Uri uri = Uri.https(Globals.STRING_HOME_URL, '/artisttracks', {
      'artist_id': widget.artist['artist_id'],
    });
    
    http.get(uri)
    .then((response) {
      /*
      To be fixed in the back-end...
      */
      this._tracks = convert.jsonDecode(response.body)['albums'];
