...
class DiscoverSearch extends StatefulWidget {
  ...
  DiscoverSearch({Key? key, required this.keyword, required this.mode})
      : super(key: key);
...
class DiscoverSearchState extends State<DiscoverSearch> {
  ...
  void didChangeDependencies() {
    ...
    if (this._init) {
      ...
      this._tileWidth = (MediaQuery.of(context).size.width -
              16 -
              (this._elementsPerRow - 1) * 8) /
  ...
  Widget get result {
    if (widget.mode is Album)
      return FadeFutureBuilder(
        future: () async {
          var str = await discover.search(widget.keyword, widget.mode);
          //print(str);
          return str;
        },
        initialWidgetBuilder: (BuildContext context) =>
        ...
        finalWidgetBuilder: (BuildContext context, Object? data) => Column(
          children: tileGridListWidgets(
              ...
              leadingSubHeader:
                  language!.STRING_SEARCH_RESULT_TOP_SUBHEADER_ALBUM,
