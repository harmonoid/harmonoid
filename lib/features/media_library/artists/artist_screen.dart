...
class _ArtistScreenState extends State<ArtistScreen> {
  static const Set<String> kSupportedImageFormats = {
    'JPG',
    'JPEG',
    'PNG',
    'WEBP',
    ...
    return HeroContentScreen(
      ...
      content: [
        ScrollViewBuilder(
          ...
          margin: margin,
          span: MediaLibraryScrollViewBuilderDataProvider(context).album.span,
          headerCount: 1,
          ...
          itemWidth: MediaLibraryScrollViewBuilderDataProvider(context).album.itemWidth,
          itemHeight: MediaLibraryScrollViewBuilderDataProvider(context).album.itemHeight,
        ...
        ListItemTable(
          ...
          onPopupMenuItemSelected: (context, i, result) async {
            await TrackMenuProvider(context, _tracks[i]).handlePopupMenuAction(
              result,
              recursivelyPopNavigatorOnDeleteIf: () => context.read<MediaLibrary>().tracksFromArtist(widget.artist).then((value) => value.isEmpty),
            );
